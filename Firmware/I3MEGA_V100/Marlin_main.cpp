...
// G5  - RESUME FROM OUTAGE,when PSU trun off,the output will not goes out immediately.- by anycubic jolly
...
// M700 - change displayer LCD2004 or 12864  
...
//  int servo_endstop_angles[] = SERVO_ENDSTOP_ANGLES;
  int servo_endstop_angles[6];
  float MY_Z_PROBE_OFFSET_FROM_EXTRUDER;
  float MY_Z_PROBE;
#endif
#ifdef BARICUDA
int ValvePressure=0;
	...
	while( NewSerial.available() > 0  && TFTbuflen < TFTBUFSIZE) {        
		  ...
		  serial3_count >= (TFT_MAX_CMD_SIZE - 1) )
	   {
		 ...
		 if(!TFTcomment_mode){
		   ...
		   {
			 TFTstrchr_pointer = strchr(TFTcmdbuffer[bufindw], 'N');
                        ...
                        //  TFT_SERIAL_ENTER();
                         ...
                         //   temp=constrain(temp,0,255);
                  			...
                  			if((!movesplanned())&&(!TFTresumingflag))
                          {
                            ...
                            {
                            if(TFTcode_seen('X'))enquecommand_P(PSTR("G28 X"));
                            if(TFTcode_seen('Y')) enquecommand_P(PSTR("G28 Y"));
                            if(TFTcode_seen('Z')) enquecommand_P(PSTR("G28 Z"));
                            }
                            else if(TFTcode_seen('C'))enquecommand_P(PSTR("G28"));                     
                          }                          
                          break;
...
void get_command()
{
  ...
  while( !card.eof()  && buflen < BUFSIZE && !stop_buffering&&!seekdataflag) {
    int16_t n=card.get();
    serial_char = (char)n;
    if(serial_char == '\n' ||
       serial_char == '\r' ||
...
//        NEW_SERIAL_PROTOCOLPGM("J14");//PRINTING DONE
        ResumingFlag=0;
...
//      }
    ...
    else
    {
      ...
      if(!comment_mode) cmdbuffer[bufindw][serial_count++] = serial_char;
              ...
              plan_set_e_position(last_position[0]); 
              plan_buffer_line (0.0,0.0,last_position[1],last_position[0],feedrate/60,active_extruder);           
              plan_set_e_position(last_position[0]);
  ...
  if(code_seen('G'))
  {
    ...
    case 4: // G4 dwell
      ...
      {                
       ...
       //  fanSpeed=255;   //OPEN FAN0  
       fanSpeed=179;       // the max fan speed set to 70%
      }     
...
//          Fan2Scan();
      ...
      #ifdef TFTmodel
      NEW_SERIAL_PROTOCOLPGM("J09");//hotbed heating done
      TFT_SERIAL_ENTER();
      #endif
        previous_millis_cmd = millis();
    #endif
      ...
      case 106: //M106 Fan On
        ...
        //   fanSpeed=constrain(code_value(),0,255);
          fanSpeed=constrain(code_value(),0,179); //179---70;204--80
        }
        else {
          fanSpeed=179;
        // fanSpeed=255;
...
void manage_inactivity()
{
  ...
  if( (millis() - previous_millis_cmd) >  max_inactive_time )
    if(max_inactive_time)
      kill();
  if(stepper_inactive_time&&!TFTresumingflag&&!PointTestFlag)  {
