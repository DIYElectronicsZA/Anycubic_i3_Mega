...
static void lcd_Outage()
{
    ...
    MENU_ITEM(submenu, MSG_NO_CARD, lcd_sdcard_menu);
    #ifdef TFTmodel
    NEW_SERIAL_PROTOCOLPGM("J02");//no sd card
    TFT_SERIAL_ENTER();
    #endif
    #endif
    END_MENU();
...
#ifdef DOGLCD && !defined(FULL_GRAPHIC_SMALL_PANEL)
#if !defined(FULL_GRAPHIC_SMALL_PANEL)
...
void lcd_update()
{
    ...
    Scancount++;
    if(((degHotend(0)<5)||((degHotend(0)>290)))&&(Scancount>60000)){ Scancount=0;NEW_SERIAL_PROTOCOLPGM("J10");TFT_SERIAL_ENTER();}//T0 unnormal
//    #ifdef LCD_HAS_SLOW_BUTTONS
